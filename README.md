...
- Windows đã tiến vào quá trình sleep tự động (hoặc còn khoảng 10 giây khi máy cung cấp countdown chính thức).
...
- Nếu countdown là `0xFFFFFFFF`, dùng `GetLastInputInfo` và **Sleep after** chỉ để chuẩn bị; không gửi Telegram từ dự đoán này.
...
- Với countdown chính thức: còn khoảng 10 giây thì gửi.
- Với Modern Standby trên máy này: task SYSTEM chỉ gửi khi `Kernel-Power Event 566` xác nhận chuyển từ session type `1` sang `3`. Qua log thực tế, tín hiệu này xuất hiện khoảng 6 giây trước Event 42/PBT.
- Nếu dự đoán idle hết hạn nhưng Windows không chuyển sleep: không gửi gì, nên không còn tin giả như `PRE_SLEEP_NOT_CONFIRMED` trước đây.

Watcher đồng thời đăng ký `PowerRegisterSuspendResumeNotification`. Khi nhận `PBT_APMSUSPEND`, callback không thực hiện mạng. Sau resume, nó đối chiếu timestamp Telegram của Event 566 rồi hiển thị `PreSleepTelegramConfirmed=True/False` trong log và **Status**.
...
Tin pre-sleep là best effort: Event 566 chỉ cho khoảng vài giây và không được Microsoft công bố như API ổn định. Task không retry để tránh gửi muộn sau resume. Startup và resume đáng tin cậy hơn vì chúng có thể chờ mạng. Nếu máy có countdown chính thức, watcher vẫn dùng `SYSTEM_POWER_INFORMATION.TimeRemaining`.
